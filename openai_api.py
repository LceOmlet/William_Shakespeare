...
- **推理过程：** 根据给定的规则和必要的数学运算，逐步推理论证，证明答案的正确性。

- **输出完整结果：** 你的输出应给出完整的演绎推理结果和最终判断。

- **不改正错误答案：** 不需要对错误的选项进行改正。需明确指出选项的错误，并着重强调其错误之处。否则可能导致后续判断中，将错误选项与你改正的正确结论混淆。

- **深入理解规则：** 为了保证推理的稳固性，深入理解并严格遵循给定的规则，这些规则是完全正确的。
...
- **术语准确性：** 判断答案中是否使用了规则中明确定义的专有名词（如级别名称、单位名称等）。将答案中使用的任何同义词或程度更低或更高的词视为无效或错误。

- **概念范畴一致性：** 判断概念范畴是否有混淆，例如题干和选项中的组织、部门与个人等并非同一范畴。若答案中的概念范畴与问题询问的范畴不同，存在作用域混淆，则该选项无效并存在逻辑一致性错误。

- **数字核对：** 严格检查数字是否正确，数字不正确即为错误。

- **输出完整推理：** 你的输出应给出完整的演绎推理结果和最终判断。
'''
...
- **推理过程：** 提供一系列推导步骤，最终确认结论对应的选项。

- **专注选项内容：** 不要将题目中的选项内容与你在推理过程中改正后的内容混淆。需专注于判断原始选项内容的正确性，否则混淆可能导致推理错误。

- **深入理解规则：** 为了确保推理的稳固性，深入理解并严格遵循给定的规则，这些规则是完全正确的。

- **逐步推理：** 进行一步一步的推理，以保证逻辑一致性。避免引入多重否定，减少语义理解的难度。

- **术语准确性：** 判断答案中是否使用了规则中明确定义的术语和专有名词（如级别名称、单位名称等）。将答案中使用的任何同义词或程度更低或更高的词视为无效或错误。

- **选项独立性：** 对于每个问题**仅有**一个可选选项。所有选项之间的关系是并列的，彼此相互独立，没有因果关系。

- **选择正确或错误的选项：**
    题干中有时会要求选择“正确的”或“符合的”选项，有时需要选择“不正确”或“不符合”的选项，务必区分清楚，再选择答案。
   - **若需选择正确选项：** 推理出正确的结论，坚决排除错误的选项，找出唯一正确的选项。
   - **若需选择错误选项：** 推理出错误的结论，坚决排除正确的选项，找出唯一错误的选项。

- **数字核对：** 严格检查数字是否正确，数字不正确则视为有误。

- **全称量词检查：** 遇到全称量词“所有”时，一定要检查规则中对应的所有实体是否均符合题干要求，选项才为正确。

- **输出完整推理：** 你的输出应给出完整的演绎推理结果。

- **题干查询范围一致性：** 判断概念范围是否有混淆，例如题干和选项中的组织、部门与个人等并非同一范围。若答案中查询的概念的范围与问题询问的范围不同，存在作用域上的混淆，则该选项无效并存在逻辑一致性错误。

- **持续检查：** 若经过上述步骤后，仍有多个可能的选项，继续按照上述步骤检查，直到排除所有不可选的选项或找到最符合要求的选项。
'''


DEDUCE_DIRECT_BASE = '''给定：

- **完全正确的规则。**
- **需要解答的问题和选项（仅有一个是可选的）。**

1. 遍历每个选项，逐个排除不可选的选项，最终根据演绎结果确定可选的选项
2. 当存在多个可选项时，返回步骤1，不过仅遍历剩余可选项
3. 以“最终答案：X”的格式给出最终选项。

你需严格遵守的规则是：

- **严谨的逻辑推理过程：** 提供一系列推导步骤，最终确认结论对应的选项，演绎逻辑务必严谨。
...
- **全称量词检查：** 遇到全称量词“所有”时，一定要检查规则中对应的所有实体是否均符合题干要求，选项才为正确。务必反复检查，保证逻辑一致性。

- **输出完整推理：** 你的输出应给出完整的演绎推理结果。

- **问题对实体的查询范围一致性：** 判断概念范围是否有混淆，例如题干和选项中的组织、部门与个人等并非同一范围。若答案中查询的概念的范围与问题询问的范围不同，存在作用域上的混淆，则该选项无效并存在逻辑一致性错误。

- **持续检查：** 若经过上述步骤后，仍有多个可能的选项，继续按照上述步骤检查，直到排除所有不可选的选项或找到最符合要求的选项。'''

CLASSIFICATION_BASE = lambda X:f"根据给出结论的末尾的判断，判别唯一可选的选项。一定直接地给出结论在结尾给出的选项，不管在结论前文中被判定为是“符合”还是“不符合”，无论被判定为“错误”还是“正确”选项，只要是直接出现在结论末尾、被判定为最终答案，则直接抄写。你的输出内容仅能为：\"分类结果：X\"。X是{', '.join(X)}中的一个，一般情况下直接抄写结论末尾的答案就好，切勿出现抄写错误和无关字符。请务必不要抄写错误！"
...
class OpenAIClient:
    ...
    def deduce_direct(self, task_text, rules_text) -> List[str]:

        rule_text = "规则：" + rules_text
        task_text = "需解答的" + task_text
        
        prompt = "\n\n\n".join([rule_text, task_text])
        
        messages = [
            { 
                "role": "system", 
                "content": DEDUCE_DIRECT_BASE
            },
            {
                "role": "user",
                "content": prompt,
